...
RANDOM_USER_API_URL = 'https://randomuser.me/api?results=500&inc=name,gender,location,email'
...
def user_data():
    if request.method == 'GET':
        ...
        for entry in results:
            ...
            user_name = entry.get('name').get('first') + \
                ' ' + entry.get('name').get('last')
            gender = entry.get('gender')
            email_ = entry.get('email')
            user = {'country': country, 'name': user_name,
                    'gender': gender, 'email': email_}
            users.append(user)
